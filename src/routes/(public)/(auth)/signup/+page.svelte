<script lang="ts">
	...
	$effect(() => {
		...
		if (signupEmail !== null) {
			const redirectTarget = `/verify-email/${buildVerifyEmailQuery(signupEmail, nextParam)}`;
	...
	const showSignupForm = $derived(
		!authState.loading && authState.user === null && signupEmail === null
	);
</script>
	...
	{#if showSignupForm}
		<div class="animate-fade-in-zoom flex w-full max-w-sm flex-col gap-8">
			...
			<div class="text-center">
				<a href="/login" class="text-sm text-muted-foreground hover:underline"> Log in instead </a>
			</div>
		</div>
	{:else}
